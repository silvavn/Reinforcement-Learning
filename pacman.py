...
def runGames( layout, pacman, ghosts, display, numGames, record, numTraining = 0, catchExceptions=False ):
  ...
  if numGames > 1:
    ...
    winRate = wins.count(True)/ float(len(wins)) if len(wins) > 0 else wins.count(True)
    print 'Average Score:', sum(scores) / float(len(scores))if len(scores) > 0 else sum(scores)
